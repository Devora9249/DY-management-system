...
export default function AddDonor({ isOpen, onClose, onAdd }) {
  ...
  const [whatsappNumber, setWhatsappNumber] = useState('');
  const [birthDate, setBirthDate] = useState('');
  const [yahrzeitDateInput, setYahrzeitDateInput] = useState('');
  const [yahrzeitNameInput, setYahrzeitNameInput] = useState('');
  ...
  // הוספת יוארצייט למערך זמני
  const handleAddYahrzeit = () => {
    if (!yahrzeitDateInput || !yahrzeitNameInput) {
      alert("יש למלא גם שם וגם תאריך");
      return;
    }
    setYahrzeits(prev => [...prev, { date: yahrzeitDateInput, name: yahrzeitNameInput }]);
    setYahrzeitDateInput('');
    setYahrzeitNameInput('');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await Axios.post("http://localhost:5678/api/donors", {
        name, donorId, address, phoneNumber, emailAddress, whatsappNumber,
        birthDate,
        yahrzeitDate: yahrzeits,  // שולח את המערך כולו
        donationAmount: Number(newAmount),
        donationDate: newDate,
        paymentMethod, frequency, duration
      });
      ...
      // איפוס השדות
      setName(''); setDonorId(''); setAddress(''); setPhoneNumber('');
      setEmailAddress(''); setWhatsappNumber(''); setBirthDate('');
      setYahrzeitDateInput(''); setYahrzeitNameInput(''); setYahrzeits([]);
      setNewDate(new Date().toISOString().split('T')[0]); setNewAmount('');
      setPaymentMethod(''); setFrequency(''); setDuration('');
    ...
    >
      ...
      <DialogContent>
        <form onSubmit={handleSubmit}>
          ...
          {/* יוארצייטים */}
          <Typography sx={{ mt: 2, mb: 1, color: '#7b1fa2', fontWeight: 'bold' }}>יוארצייטים</Typography>
          {yahrzeits.map((y, i) => (
            <Box key={i} sx={{ display: 'flex', gap: 1, mb: 1 }}>
              <TextField type="date" value={y.date} InputLabelProps={{ shrink: true }} size="small" disabled fullWidth />
              <TextField value={y.name} size="small" disabled fullWidth />
            </Box>
          ))}
          <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
            <TextField
              ...
              InputLabelProps={{ shrink: true }}
              size="small"
              value={yahrzeitDateInput}
              onChange={e => setYahrzeitDateInput(e.target.value)}
              fullWidth
            />
            <TextField
              placeholder="שם היוארצייט"
              size="small"
              value={yahrzeitNameInput}
              onChange={e => setYahrzeitNameInput(e.target.value)}
              fullWidth
            />
            <Button variant="outlined" size="small" onClick={handleAddYahrzeit}>הוסף</Button>
          </Box>
